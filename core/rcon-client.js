...
export default class RconClient extends EventEmitter {
  ...
  onData(data) {
    ...
    for (const packet of packets) {
      ...
      logger.debug(`Processing decoded packet: ${this.decodedPacketToString(decodedPacket)}`);
  ...
  write(type, body) {
    return new Promise((resolve, reject) => {
      ...
      logger.verbose(`Sending packet: ${this.bufToHexString(encodedPacket)}`);
      this.client.write(encodedPacket);

      if (type !== SERVERDATA_AUTH) {
        logger.verbose(`Sending empty packet: ${this.bufToHexString(encodedEmptyPacket)}`);
  ...
  onClose(hadError) {
    ...
    logger.verbose(`Socket closed ${hadError ? 'without' : 'with'} an error.`);

    if (this.autoReconnect) {
      logger.verbose(`Sleeping ${this.autoReconnectDelay}ms before reconnecting.`);
